...
namespace
{
    ...
    class TestDataArbitraryPrime
    {
    public:
        constexpr static size_t Prime{ 101 };
    };

    class TestDataBinary
    {
    public:
        constexpr static size_t Prime{ 2 };
    };
    
    template <typename T>
    class PrimeFieldsTests : public testing::Test
    {
    public:
        constexpr static size_t arbitrary_nonzero_element{ T::Prime / 2 };
    };

    using PrimeFieldsSchemas = testing::Types<
		TestDataArbitraryPrime,
        TestDataBinary
    >;

    TYPED_TEST_SUITE( PrimeFieldsTests, PrimeFieldsSchemas );
}

// Functionality

TYPED_TEST( PrimeFieldsTests, AllInversesAreInverses )
{
	for( integer non_zero_element = 1; non_zero_element < TypeParam::Prime; non_zero_element++ )
	{
		ElementOfFiniteFieldP<TypeParam::Prime> a{ non_zero_element };
        EXPECT_EQ( (a*a.FindMultiplicativeInverse()), 1 ) << "New value: " << a.value << "Inverse value: " << a.FindMultiplicativeInverse().value;
	}
}

TYPED_TEST( PrimeFieldsTests, InverseFindingClassMethodAndOriginalImplAgree )
{
	for( integer non_zero_element = 1; non_zero_element < TypeParam::Prime; non_zero_element++ )
    {
        ElementOfFiniteFieldP<TypeParam::Prime> a{ non_zero_element };
        EXPECT_EQ( FindMultiplicativeInverse_Simple<TypeParam::Prime>(a), a.FindMultiplicativeInverse() );
    }
}

TYPED_TEST( PrimeFieldsTests, ClassIsConstexprInstantiable )
{
	constexpr ElementOfFiniteFieldP<TypeParam::Prime> a{ arbitrary_nonzero_element };
}

TYPED_TEST( PrimeFieldsTests, AdditionInvarientHelperConstexpr )
{
	constexpr auto zero{ ElementOfFiniteFieldP<TypeParam::Prime>::GetAdditionInvarient() };
}

TYPED_TEST( PrimeFieldsTests, AdditionInvariantIsZero )
{
	const auto zero{ ElementOfFiniteFieldP<TypeParam::Prime>::GetAdditionInvarient() };

	EXPECT_TRUE( zero.IsZero() );
	EXPECT_FALSE( zero.IsOne() );
}

TYPED_TEST( PrimeFieldsTests, MultiplicativeInvariantIsOne )
{
	auto one{ ElementOfFiniteFieldP<TypeParam::Prime>::GetMultiplicativeInvarient() };	

	EXPECT_FALSE( one.IsZero() );
	EXPECT_TRUE( one.IsOne() );
}

TYPED_TEST( PrimeFieldsTests, EqualityOperatorIsConstexpr )
{
	constexpr auto one{ ElementOfFiniteFieldP<TypeParam::Prime>::GetMultiplicativeInvarient() };
	constexpr auto zero{ ElementOfFiniteFieldP<TypeParam::Prime>::GetAdditionInvarient() };

    constexpr bool eq{ one == zero };
    EXPECT_FALSE( eq );
}
